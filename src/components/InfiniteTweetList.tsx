...
export function InfiniteTweetList({ tweets, isError, isLoading, fetchNewTweets, hasMore }: InfiniteTweetListProps) {
  ...
  if (tweets == null || tweets?.length === 0) {
    return (
      <h2 className="my-4 text-center text-2xl text-gray-400">No Tweets</h2>
    )
  }
  ...
  return <li className="flex gap-4 border-b px-4 py-4">
    ...
    <div className="flex flex-grow flex-col">
      <div className="flex gap-1">
        <Link className="font-bold hover:underline focus-visible:underline outline-none" href={`/profiles/${user.id}`}>
          {user.name}
        </Link>
